...
CREATE TABLE public.tts_audio (
  ...
  -- Ensure we don't store duplicates per user
  UNIQUE (user_id, audio_key)
);

-- Add indexes for performance
CREATE INDEX tts_audio_user_id_idx ON public.tts_audio(user_id);
CREATE INDEX tts_audio_audio_key_idx ON public.tts_audio(audio_key);

-- Set up RLS policies
ALTER TABLE public.tts_audio ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can only access their own TTS audio"
  ON public.tts_audio
  FOR ALL
  USING (auth.uid() = user_id);
